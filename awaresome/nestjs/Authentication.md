...
#### Creating an authentication module 创建一个认证模块
...
#### Enable authentication globally 启用全局守卫
...
必须提供一种机制来确保部分的路由不需要认证。为此，我们可以使用 `SetMetadata` 装饰器工厂函数。
...
#### Passport integration 集成`Passport`
