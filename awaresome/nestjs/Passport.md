### Passport (authentication) 密码认证策略
...
- Authenticate a user by verifying their "credentials" (such as username/password, JSON Web Token ([JWT](https://jwt.io/)), or identity token from an Identity Provider)；通过凭证用户是否有效
- Manage authenticated state (by issuing a portable token, such as a JWT, or creating an [Express session](https://github.com/expressjs/session))； 管理认证状态
- Attach information about the authenticated user to the `Request` object for further use in route handlers； 📌 将认证用户的信息附加到 `Request` 对象中，以便在路由处理程序中使用
...
1. A set of options that are specific to that strategy 某个策略的配置项. For example, in a JWT strategy, you might provide a secret to sign tokens.
2. A "verify callback", which is where you tell Passport how to interact with your user store (where you manage user accounts). Here, you verify whether a user exists (and/or create a new user), and whether their credentials are valid. The Passport library expects this callback to return a full user if the validation succeeds, or a null if it fails (failure is defined as either the user is not found, or, in the case of passport-local, the password does not match). 一个认证回调，是一个函数，它告诉 Passport 如何与用户存储（例如，数据库）交互。在这种情况下，它验证用户是否存在（或在 passport-local 中，密码是否匹配）。如果验证成功，它将返回完整的用户对象，如果验证失败，则返回 null。
...
你需要在 `@nestjs/passport` 中扩展 `PassportStrategy` 类。你通过调用 `super()` 方法来传递选项（例如，在 JWT 策略中，你可能会提供一个密钥来签名令牌）。提供实现了`validate()`的回调方法。
...
#### Implementing Passport local 实现一个本地密码验证策略
...
#### Built-in Passport Guards 内置的密码守卫
