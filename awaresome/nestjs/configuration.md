...
应用程序通常在不同的**环境**中运行。通常，本地环境依赖于特定的数据库凭据，仅在本地 DB 实例上有效。生产环境则使用单独的 DB 凭据。由于配置变量会改变，最好的做法是使用[存储配置变量](https://12factor.net/config)
...
通过全局`process.env`定义的环境变量可被`Nodejs`访问到。我们可以通过设置每个环境的环境变量单独来解决这个问题。这可以通过在每个环境中单独设置`.env`文件来实现。这种方法很快就会变得很繁琐，特别是在开发和测试环境中，这些值需要轻松地进行模拟和/或更改。
...
在 Node.js 应用程序中，通常使用`.env`文件，其中包含键值对，每个键表示一个特定的值。运行不同的环境是通过切换使用正确的`.env`文件来实现的。
...
```typescript
ConfigModule.forRoot({
  ignoreEnvFile: true, // 不使用环境变量文件
  // FIXME docker部署下如何配置呢
})
```
...
#### Custom configuration files 自定义环境文件
