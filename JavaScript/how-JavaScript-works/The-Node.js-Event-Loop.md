...
事件循环是 Node.js 能够执行非阻塞 I/O 操作的原因——尽管默认情况下使用单个 JavaScript 线程——它通过尽可能将操作卸载到系统内核来实现这一点。

Since most modern kernels are multi-threaded, they can handle multiple operations executing in the background. When one of these operations completes, the kernel tells Node.js so that the appropriate callback may be added to the **poll** queue to eventually be executed. We'll explain this in further detail later in this topic.
由于大多数现代内核都是多线程的，它们可以处理多个后台操作。当这些操作中的一个完成时，内核会通知 Node.js，以便将适当的回调添加到 poll 队列中，以便最终执行。我们将在本主题的稍后部分进一步解释这一点。

## Event Loop Explained

When Node.js starts, it initializes the event loop, processes the provided input script (or drops into the [REPL][], which is not covered in this document) which may make async API calls, schedule timers, or call `process.nextTick()`, then begins processing the event loop.
当 Node.js 启动时，它会初始化事件循环，处理提供的输入脚本（或进入 REPL 模式，本文档不涵盖此内容），该脚本可能会进行异步 API 调用、安排计时器或调用 `process.nextTick()` ，然后开始处理事件循环。
