...
Dart 的 Event Loop 并不是独立于当前运行程序之外的东西，而是 Dart 运行时的一部分，特别是在 Dart VM（虚拟机）中。Event Loop 是一种编程模式，用于处理异步操作和事件的调度。在 Dart 中，Event Loop 使得单线程可以高效地处理大量异步事件和操作。

以下是 Dart Event Loop 一些关键点：

1. **单线程模型**：Dart 的 Event Loop 运行在 Dart VM 的单线程上。这意味着所有的 Dart 代码都在同一个线程上执行。

2. **异步操作**：Dart 使用 `Future` 和 `Stream` 来处理异步操作。这些异步操作不会阻塞主线程，而是在后台执行，并在完成后将结果或错误回调到 Event Loop。

3. **微任务和宏任务**：Dart 的 Event Loop 包括微任务（microtask）和宏任务（macrotask）。微任务包括回调队列，比如 `then`、`catchError` 和其他注册的回调。宏任务包括定时器、I/O 操作、UI 渲染等。

4. **执行顺序**：Dart 的 Event Loop 首先执行所有的微任务，直到队列为空。然后，它执行一个宏任务，接着再次检查微任务队列。这个过程循环进行，直到所有的任务都完成。

5. **非阻塞 I/O**：Dart 的 I/O 操作是异步和非阻塞的，它们通过 Event Loop 来处理，从而不会阻塞主线程。

6. **与 Flutter 的关系**：在 Flutter 框架中，Dart 的 Event Loop 也用于处理 UI 的更新和渲染，确保 UI 的流畅性和响应性。

7. **跨平台执行**：虽然 Dart VM 使用单线程的 Event Loop，但 Dart 也可以编译为 JavaScript 或原生代码运行在其他平台上，这些平台可能有自己的异步处理机制。

总之，Dart 的 Event Loop 是 Dart 运行时的一个核心特性，它使得 Dart 能够有效地处理异步操作，同时保持代码的简洁性和易读性。
...
![A figure showing events being fed, one by one, into the event loop](image.png)
...
![Figure showing async events being added to an event loop andholding onto a callback to execute later.](image-1.png)
...
for native I/O in between](image-2.png)
...
![A more general figure showing that any isolate runs some code, optionally responds to events, and then exits](image-4.png)
...
![A figure showing events being fed, one by one, into the event loop](image-5.png)
...
![A figure showing a tap handler with a too-long execution time](image-6.png)
...
![A figure showing a main isolate and a simple worker isolate](image-7.png)
